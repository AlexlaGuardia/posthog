...
from .run_evaluation import BooleanEvalResult, LLMJudgeResult
...
class TestFormatTraceForJudge:
    """`format_trace_for_judge` delegates to the shared `text_repr` formatter (the same one the
    trace view and summarizer use). These assert the integration — that the judge receives the
    trace's events and content via that formatter — not the formatter's exact tree art, which is
    covered by text_repr's own tests."""
...
class TestBuildTraceHogGlobals:
    def test_builds_events_with_trace_level_input_output(self):
        events = [
        ...
        assert globals_dict["input"] == "first question"
        assert globals_dict["output"] == "final answer"
...
class TestFetchTraceForEvaluation:
    ...
    def test_skips_when_no_events_found(self, setup_data):
        ...
        with patch("posthog.temporal.ai_observability.run_trace_evaluation._count_trace_events", return_value=0):
            outcome = fetch_trace_for_evaluation(team.id, "trace-123", datetime.now(UTC))
...
class TestExecuteTraceHogEvalActivity:
    ...
    async def test_runs_hog_against_trace_globals(self, setup_data):
        ...
        bytecode = compile_hog("return length(events) = 2 and output = 'final answer'", "destination")
...
class TestEmitTraceEvaluationEventActivity:
    ...
    async def test_emits_event_targeting_trace(self, setup_data):
        ...
        with patch("posthog.temporal.ai_observability.run_trace_evaluation.Team.objects.get", return_value=team):
            with patch(
                "posthog.temporal.ai_observability.run_trace_evaluation.capture_internal_routed"
            ) as mock_capture:
