...
# How long to wait after the first matching generation before pulling the trace. A turn in an
# LLM conversation rarely exceeds a few minutes even with heavy tool usage, so by then the
# trace can be assumed settled. Events arriving later are not re-evaluated.
TRACE_EVALUATION_DEFAULT_WINDOW_SECONDS = 30 * 60
TRACE_EVALUATION_MAX_WINDOW_SECONDS = 2 * 60 * 60
...
# Written against ai_events; execute_with_ai_events_fallback rewrites it for the events table
# when needed. HAVING makes a zero count return no rows, which both triggers the events-table
...
class RunTraceEvaluationInputs:
    ...
    window_seconds: int = TRACE_EVALUATION_DEFAULT_WINDOW_SECONDS
...
def _build_trace_skip_result(allows_na: bool, skip_reason: str) -> LLMJudgeResult:
...
def build_trace_hog_globals(trace: LLMTrace, trace_id: str) -> dict[str, Any]:
    ...
    `input`/`output` mirror the single-event globals — the trace's first non-empty input and
    last non-empty output — so simple sources work unchanged on both targets.
    """
    ...
    for event in trace.events or []:
        ...
        input_val = coerce_hog_io_value(input_raw)
        output_val = coerce_hog_io_value(output_raw)
        if not first_input and input_val:
            first_input = input_val
        if output_val:
...
def execute_trace_llm_judge_activity(inputs: ExecuteTraceEvaluationInputs) -> LLMJudgeResult:
...
@temporalio.activity.defn
async def execute_trace_hog_eval_activity(inputs: ExecuteTraceEvaluationInputs) -> LLMJudgeResult:
...
class RunTraceEvaluationWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: RunTraceEvaluationInputs) -> WorkflowResult:
        ...
        window_seconds = min(max(inputs.window_seconds, 0), TRACE_EVALUATION_MAX_WINDOW_SECONDS)
        if window_seconds:
        ...
        if not evaluation.get("enabled", True):
            disabled_result: WorkflowResult = {
                "verdict": None,
                "skipped": True,
                "skip_reason": "evaluation_disabled",
        ...
        if evaluation_type == "hog":
            ...
            try:
                ...
                skip_result = await handle_llm_judge_activity_error(e, evaluation, evaluation_type)
