...
class TestAIEventsUsageReport(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    def test_posthog_code_credits_only_counts_posthog_code_events(self, mock_region: MagicMock) -> None:
        ...
        # Signals event — should appear only in signals credits, never in posthog_code credits.
        _create_event(
            event="$ai_generation",
            team=analytics_team,
            distinct_id="user_signals",
        ...
        # posthog_code: 2.0 USD * 100 * 1.2 = 240
        self.assertEqual(posthog_code_result, [(self.org_1_team_1.id, 240)])
        # signals: 1.0 USD * 100 * 1.2 = 120
        self.assertEqual(signals_result, [(self.org_1_team_1.id, 120)])
